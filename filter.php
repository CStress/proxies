<?php
////Proxy checker. Written and made by CStress IP Stresser Booter
if (!isset($argv[3]))
{
    ...
    if ($pid == -1) {
        ...
        while (true) {
            ...
            foreach ($part[$i] as $line) {
                ...
                $ch         = curl_init();
                $curlConfig = array(
                    CURLOPT_URL => "http://pastebin.com/raw.php?i=GNsjKYy5",
                ...
                $result = curl_exec($ch);
                curl_close($ch);
                if (strpos($result, "formyproxycheckerandyquezhasabigdick") !== FALSE) {
